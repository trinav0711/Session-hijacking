...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    $sql=$link->prepare("SELECT * FROM users WHERE username = ?"); // *Revised*
    $sql->bind_param("ss", $user); // *Revised*
    $sql->execute(); // *Revised*
	...
	if(mysqli_num_rows($res) == 0){
        ...
        $row = $res->fetch_assoc(); // *Revised*
        $hashed_password = $row['password']; // *Revised*

        if(password_verify($pass, $hashed_password)){ // *Revised*
        // Username and password is correct, so start a new session.
    		session_start();

            // *Revised*: Assign a new session ID on every login.
            ...
            $password_err = "The username and/or password you entered was not valid."; // *Revised*
        } // *Revised*
    }
